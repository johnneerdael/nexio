...
1. Start with the rows you actually want people to browse.
2. Move broad discovery rows below the rows that already match your account's taste.
3. Hide rows that repeat the same job.
4. Save, sync, and check Home in the TV app.

## Next step

If the rows are in the right order but the cards still feel hard to read, move to [Universal Formatter](/customize/universal-formatter).
