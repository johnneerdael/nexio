...
5. Check [Security and Data](./security-and-data.md) before sharing access or storing secrets.

## What to expect
- Changes are saved to the account, not to one TV.
- Save actions are meant to be followed by sync, so the same account can see the same configuration elsewhere.
