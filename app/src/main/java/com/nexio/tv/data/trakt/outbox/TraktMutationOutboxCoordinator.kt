...
@Singleton
class TraktMutationOutboxCoordinator @Inject constructor(
    private val worker: TraktMutationOutboxWorker,
    adapters: Set<@JvmSuppressWildcards TraktMutationAdapter>
) {
    companion object {
        private const val TAG = "TraktOutboxCoordinator"
    }
        ...
        while (true) {
            ...
            when (execution) {
                is TraktMutationExecutionResult.Success -> {
                    ...
                    runCatching {
                        adapter.reconcileSuccess(lease.envelope)
                    }.onFailure { error ->
                        Log.w(
                            TAG,
                            "Failed to reconcile successful Trakt mutation ${lease.envelope.id}: ${error.message}"
                        )
                    }
                }
                ...
                is TraktMutationExecutionResult.Failure -> {
                    ...
                    if (settlement is TraktMutationSettlement.TerminalFailure && settled != null) {
                        runCatching {
                            adapter.rollbackToServerTruth(settled, settlement)
                        }.onFailure { error ->
                            Log.w(
                                TAG,
                                "Failed to rollback terminal Trakt mutation ${settled.id}: ${error.message}"
                            )
                        }
                    }
                }
