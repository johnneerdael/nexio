...
/**
 * Pure-logic mapping from ExoPlayer's video [ColorInfo] + a user [HdrMode] preference
 * to a [CaptureMode]. JVM-testable; the Player.Listener wiring in
 * PlayerRuntimeControllerInitialization calls this on every onTracksChanged to decide
 * whether to install the SDR or HDR effect.
 */
object FormatDetector {
    ...
    fun detect(
        ...
        // Server-side gate: stock HyperHDR can't decode P010 frames, so even on a wide-color
        // device with an HDR source we must downconvert to NV12. The fork advertises P010
        // support via serverinfo.flatbuffer.imageFormats; absence == not supported.
        if (!serverSupportsP010) return CaptureMode.SDR_NV12
