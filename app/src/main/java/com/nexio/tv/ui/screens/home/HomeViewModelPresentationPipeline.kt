...
// --- Plan: Bug A — Task A3 ---
//
// Pre-warming the MediaClipStore for screensaver items so the synchronous
// TrailerResolver consulted by HomeResolvedDisplayMapper can find candidates
// for each ResolvedDisplayItem.trailer instead of returning ItemLookup refs
// that the screensaver session can't dereference at playback time.

/**
 * Pure data class for a single screensaver-pool warm request. Kept outside
 * the extension function so tests can construct candidates without needing
 * a `HomeViewModel` instance.
 */
internal data class ScreensaverWarmCandidate(
    val itemId: String,
    val title: String,
...
private const val SCREENSAVER_TRAILER_WARM_THROTTLE_MS = 150L

/**
 * Sequential, indexed-for trailer warming for the screensaver candidate pool.
 *
 * Iterates `candidates` one at a time and calls
 * `metadataRouterFacade.fetchTrailer(...)` for each — TmdbTrailerMetadataAdapter
 * and TvdbTrailerMetadataAdapter write the resolved candidates into
 * MediaClipStore as a side effect. Throttled to keep aggregate TMDB load
 * bounded (84 items × 150ms = ~13s warm budget at one in-flight request at
 * a time).
 *
 * CLAUDE.md rule #4 — indexed-for instead of `.forEach { suspend ... }`
 * (no Iterator captured into the continuation).
 * CLAUDE.md rule #6 — no `supervisorScope { launch { ... } }` fan-out so
 * the candidate list does not get pinned into N parallel continuations.
 *
 * `onItemFetched` is a test seam; production callers pass `{}`.
 */
internal suspend fun warmScreensaverTrailerCache(
    candidates: List<ScreensaverWarmCandidate>,
    facade: MetadataRouterFacade,
    languageTag: String?,
    throttleMs: Long = SCREENSAVER_TRAILER_WARM_THROTTLE_MS,
    onItemFetched: (itemId: String) -> Unit = {}
) {
    for (i in candidates.indices) {
        val candidate = candidates[i]
        try {
            facade.fetchTrailer(
                metadataRequest = screensaverTrailerWarmMetadataRequest(
                ...
                title = candidate.title,
                year = candidate.releaseInfo?.take(4)?.takeIf { it.length == 4 },
                tmdbId = candidate.tmdbId,
                type = candidate.apiType,
                contentId = candidate.itemId,
                fallbackYtIds = listOfNotNull(candidate.fallbackYtId?.trim()?.takeIf { it.isNotBlank() })
            )
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
            // Per-item resilience — one TMDB error must not abort the
            // remaining warmups. The screensaver session can survive
            // partial coverage but cannot survive job cancellation due to
            // a single 429 or transient DNS failure.
            Log.w(
                "ScreensaverTrailerWarm",
                "fetchTrailer failed for itemId=${candidate.itemId}: ${e.javaClass.simpleName}: ${e.message}"
            )
        }
        onItemFetched(candidate.itemId)
        if (throttleMs > 0L) delay(throttleMs)
    }
}

private fun screensaverTrailerWarmMetadataRequest(
    itemId: String,
    title: String,
    releaseInfo: String?,
    apiType: String,
...
/**
 * Launch (or relaunch) the screensaver trailer warmer for the current
 * screensaver candidate pool. No-op if non-playback work is currently
 * disallowed (HomePlaybackWorkGate semantics). After the warmer finishes,
 * re-publishes the screensaver surface so the freshly cached MediaClipStore
 * entries land as ResolvedDisplayItem.trailer.selectedPlaybackRef.
 *
 * Caller: publishTmdbTrendingScreensaverSurface (Task A4).
 */
internal fun HomeViewModel.refreshScreensaverTrailerCachePipeline(
    candidatesProvider: () -> List<ScreensaverWarmCandidate>
) {
    if (!isNonPlaybackHomeWorkAllowed()) return
    screensaverTrailerWarmJob?.cancel()
    screensaverTrailerWarmJob = viewModelScope.launch {
        val candidates = candidatesProvider()
        ...
        republishScreensaverSurfaceAfterWarm()
    }
}
