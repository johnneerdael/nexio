...
@Composable
fun OmdbSettingsScreen(
    viewModel: OmdbSettingsViewModel = hiltViewModel(),
    onBackPress: () -> Unit
) {
    BackHandler { onBackPress() }

    SettingsStandaloneScaffold(
        title = stringResource(R.string.omdb_title),
        subtitle = stringResource(R.string.omdb_subtitle)
    ) {
        OmdbSettingsContent(viewModel = viewModel)
    }
}

@Composable
fun OmdbSettingsContent(
    viewModel: OmdbSettingsViewModel = hiltViewModel(),
    ...
    val invalidApiKeyMsg = stringResource(R.string.omdb_invalid_api_key)

    LaunchedEffect(Unit) {
        viewModel.validationError.collect { error ->
            if (error == OmdbValidationError.InvalidApiKey) {
    ...
    Column(
        ...
        SettingsGroupCard(
            ...
            LazyColumn(
                ...
                item(key = "omdb_enabled") {
                    SettingsToggleRow(
                        title = stringResource(R.string.omdb_enable_title),
                        subtitle = stringResource(R.string.omdb_enable_subtitle),
                ...
                item(key = "omdb_account_status") {
                    SettingsActionRow(
                        title = stringResource(R.string.omdb_api_key_title),
                        subtitle = stringResource(R.string.omdb_api_key_subtitle),
                        value = maskOmdbApiKey(uiState.apiKey, stringResource(R.string.mdblist_not_set)),
                        enabled = true,
...
private fun OmdbApiKeyDialog(
    ...
    NexioDialog(
        onDismiss = onDismiss,
        title = stringResource(R.string.omdb_dialog_title),
        subtitle = stringResource(R.string.omdb_dialog_subtitle),
        width = 700.dp
    ) {
        Card(
            ...
            Box(modifier = Modifier.padding(horizontal = 14.dp, vertical = 12.dp)) {
                BasicTextField(
                    ...
                    decorationBox = { innerTextField ->
                        if (value.isBlank()) {
                            Text(
                                text = stringResource(R.string.omdb_dialog_placeholder),
...
private fun maskOmdbApiKey(key: String, notSetLabel: String): String {
