...
object AioBuiltInFormatters {
    ...
    val UNIVERSAL = AioTemplateDefinition(
        id = "universal",
        nameTemplate = """{stream.resolution::exists["{stream.resolution::replace('2160p','[[icon:4k]] ')::replace('1440p','[[icon:2k]] ')::replace('1080p','[[icon:fullhd]] ')::replace('720p','[[icon:hd]] ')::replace('576p','[[icon:sd]] ')::replace('480p','[[icon:sd]] ')}"||""]}{stream.title::exists["{stream.title::title::truncate(30)}"||"?"]}{metadata.queryType::=movie::and::stream.year::exists[" ({stream.year})"||""]}{metadata.queryType::exists::isfalse::and::stream.seasonEpisode::exists::isfalse::and::stream.year::exists[" ({stream.year})"||""]}{stream.seasonEpisode::exists[" ({stream.seasonEpisode::join('')})"||""]}{stream.seadexBest::istrue[" 🏆"||""]}{stream.seadex::istrue::and::stream.seadexBest::isfalse[" 🥈"||""]}{stream.message::~Download["{tools.removeLine}"||""]}""",
        descriptionTemplate = """
{stream.visualTags::exists["{stream.visualTags::join(' ')::replace('Dolby Vision','[[icon:dovi]]')::replace('DoVi','[[icon:dovi]]')::replace('DV','[[icon:dovi]]')::replace('HDR10+','[[icon:hdr10]]')::replace('HDR10','[[icon:hdr10]]')::replace('HDR','[[icon:hdr10]]')}"||""]}{stream.visualTags::exists::and::stream.audioTags::exists[" "||""]}{stream.audioTags::exists["{stream.audioTags::join(' ')::replace('Atmos','[[icon:atmos]]')::replace('TrueHD','[[icon:truehd]]')::replace('DTS-HD MA','[[icon:dtshd]]')::replace('DTS:X','[[icon:dtsx]]')::replace('DD+','[[icon:ddp]]')::replace('DD','[[icon:dd]]')::replace('EAC3','[[icon:ddp]]')::replace('AC3','[[icon:dd]]')::replace('DTS','[[icon:dts]]')}"||""]}
💾 {stream.size::>0["{stream.size::bytes}"||"Unknown"]}{stream.size::>0::and::stream.duration::>0[" • "||""]}{stream.duration::>0["⏱️ {stream.duration::time}"||""]}
{stream.filename::~NF["[[icon:netflix]] Netflix"||""]}{stream.filename::~DSNP["[[icon:disneyplus]] Disney+"||""]}{stream.filename::~HMAX["[[icon:hbo]] HBO Max"||""]}{stream.filename::~.MAX.["[[icon:max]] Max"||""]}{stream.filename::~AMZN["[[icon:prime]] Amazon"||""]}{stream.filename::~APTV["[[icon:appletv]] Apple TV+"||""]}{stream.filename::~ATVP["[[icon:appletv]] Apple TV+"||""]}{stream.filename::~PMTP["[[icon:paramount]] Paramount+"||""]}{stream.filename::~PCOK["[[icon:peacock]] Peacock"||""]}{stream.filename::~CRTC["[[icon:crunchyroll]] Crunchyroll"||""]}{stream.filename::~CR.["[[icon:crunchyroll]] Crunchyroll"||""]}{stream.filename::~NF::or::stream.filename::~DSNP::or::stream.filename::~HMAX::or::stream.filename::~.MAX.::or::stream.filename::~AMZN::or::stream.filename::~APTV::or::stream.filename::~ATVP::or::stream.filename::~PMTP::or::stream.filename::~PCOK::or::stream.filename::~CRTC::or::stream.filename::~CR.::and::service.name::exists[" • "||""]}{service.name::~Real-Debrid["[[icon:realdebrid]] Real-Debrid"||""]}{service.name::~Premiumize["[[icon:premiumize]] Premiumize"||""]}{service.name::~Real-Debrid::isfalse::and::service.name::~Premiumize::isfalse::and::service.name::exists["{service.name}"||""]}
📄 {stream.filename::exists["{stream.filename}"||"—"]}
""".trimIndent()
    )
