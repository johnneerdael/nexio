...
class AccountAddonSecretPayloadValidatorsTest {
    @Test
    fun `requireValidV2Transport accepts manifest suffix payload`() {
    ...
    @Test
    fun `requireValidV2Transport throws when kind is not manifest suffix`() {
    ...
    @Test
    fun `requireValidV1Secret accepts params payload`() {
        val payload = AccountAddonSecretPayload(
            kind = "query_params",
    ...
    @Test
    fun `requireValidV1Secret accepts path segment payload`() {
        val payload = AccountAddonSecretPayload(
            kind = "path_segment",
    ...
    @Test
    fun `requireValidV1Secret throws when params and path segment are empty`() {
        val error = assertThrows(IllegalStateException::class.java) {
            AccountAddonSecretPayload().requireValidV1Secret(
                secretRef = "addon:legacy_addon",
                addonUrl = "https://legacy.example.com"
            )
        }
