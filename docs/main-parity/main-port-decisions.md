...
| Detail canonical fallback | MetadataRouterFacade, StableIdBundleResolver, ProviderPlanExecutor, ProviderPlanRunner, FieldResolver, stream title guard |
...
| Metadata router design | MetadataRouter, StableIdBundleResolver, ProviderPlanExecutor, ProviderPlanRunner, FieldResolver |
| Provider contracts/blueprints | IntegrationRuntime operation shapes, provider adapters, and contract tests |
| Subtitles | Shared subtitle discovery/selection/download path and player subtitle presentation |
| Subtitle burn-in | Shared burn-in launch policy, current episode context, and playback subtitle pipeline |
| Trailers | Shared trailer runtime/provider path and trailer player state handling |
| Screensaver/settings | IdleScreensaverController, LayoutPreferenceDataStore, and settings repositories |
