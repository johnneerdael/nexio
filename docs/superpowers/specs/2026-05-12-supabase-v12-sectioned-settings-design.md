...
Contract v11 protects account sync surfaces with timestamp watermarks and removes the Wyzie integration from the server-side synced settings model, but account settings are still stored and written as one large JSONB document in `account_settings_public.settings_payload`. That leaves Android and `nexio-web` sharing one source-of-truth blob even though each client owns and understands only part of the document.

Contract v12 splits account settings into sectioned JSONB rows. Each section becomes its own source-of-truth boundary with its own payload, revision, update timestamp, and conflict behavior. Addons, secrets, profile settings blobs, and profile auth tokens keep their v11 surfaces unless a later contract changes them.
...
- Keep v11 clients working during rollout through adapter RPCs.
- Avoid changing profile ownership semantics in this contract.

## Non-Goals

- Do not move profile 1/default-profile settings out of the account-settings compatibility shape in v12.
- Do not normalize every setting into scalar SQL columns.
- Do not change account addons, account secrets, profile settings blobs, or profile auth token contracts beyond including them in the v12 snapshot envelope.
- Do not remove v11 RPCs during initial rollout.

## Current Problem

Today the v11 account snapshot has separate surfaces for settings, addons, and secrets, but the settings surface is still one JSONB payload:
...
Use an authoritative section table plus backward-compatible v11 adapter RPCs.
...
sync_pull_account_settings_sections_v12()
```

`sync_pull_account_snapshot_v12()` returns the v11 addons and secrets envelope, but sectioned account settings:
...
V11 RPCs remain available during rollout and become adapters over `account_settings_sections`.

### V11 Pull Adapter

`sync_pull_account_snapshot_v11()` reads `account_settings_sections`, merges known section rows into the legacy settings object, and returns the normal v11 envelope:
...
The adapter should preserve v11 behavior for addons and secrets.

### V11 Push Adapter

`sync_push_account_settings_v11(...)` accepts the legacy full settings payload and explodes known subtrees into section rows. The old `p_changed_paths` can be mapped to affected section keys.

Behavior:

- If a v11 payload contains multiple known sections, each extracted section is written separately.
- If a v11 push has a stale base relative to any target section, return a stale/conflict outcome compatible with existing v11 clients.
...
- V12 clients never write `account_settings_public.settings_payload`.
...
4. Add v12 pull/push RPCs.
5. Update v11 pull/push RPCs to adapter behavior.
...
settings_payload.integrations.tvdb -> integrations.tvdb
...
- Persist only dirty sections through v12 RPCs.
- Bootstrap from section rows, compose `PortalSettings`, and retain unknown remote sections in sync state.
- Stop using full settings-object writes once v12 is enabled.

The v12 client invariant is: no client sends a complete account settings object as a remote write.
...
- V12 pull returns sections with independent `updated_at_ms`.
- V12 single-section push updates only that section.
- V12 batch push can partially apply sections.
- Stale base on `formatter` does not block `integrations.subtitleTranslation`.
- V11 pull adapter reconstructs the legacy payload.
- V11 push adapter explodes a legacy payload into section rows.

Android tests:

- V12 snapshot with unknown section does not fail.
...
- V11 fallback is not used once v12 is enabled.

## Observability

Add temporary rollout visibility:

- Source labels: `android-v12`, `web-v12`, `v11-adapter`.
...
1. Deploy additive Supabase migration and v12 RPCs.
2. Backfill section rows.
3. Switch v11 RPCs to adapter behavior.
4. Update Android to read/write v12 sections.
5. Update `nexio-web` to read/write v12 sections.
6. Monitor source labels and stale conflicts.
7. After both clients are stable on v12, plan a later contract to retire v11 adapters and decide the future of `account_settings_public`.
...
- Whether v11 adapter push should keep path-level `field_conflict` behavior or collapse to stale-base on affected section conflicts. Default recommendation: preserve compatible `field_conflict` responses where existing clients already branch on them.
