...
reuse_device_credential_id uuid null references public.device_credentials(id) on delete set null
...
test("durable auth migration supports logical credential reuse handoffs", () => {
  assert.match(
    migrationContractText,
    /add column if not exists reuse_device_credential_id uuid null references public\.device_credentials\(id\) on delete set null/i,
  );
  assert.match(
    migrationContractText,
    /p_reuse_device_credential_id uuid default null/i,
  );
  assert.match(
    migrationContractText,
    /reuse_device_credential_id = p_reuse_device_credential_id/i,
  );
  assert.match(
    migrationContractText,
    /IF v_handoff\.reuse_device_credential_id IS NOT NULL THEN/i,
  );
  assert.match(
    migrationContractText,
