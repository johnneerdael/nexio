-- Add side-by-side addon transport v2 metadata.
--
-- v1 clients keep receiving url/manifest_url/secret_ref exactly as before.
-- v2 clients can use transport_base_url + transport_secret_ref, where the
-- secret payload is { kind: "manifest_suffix_v1", suffix: "/.../manifest.json" }.

alter table public.account_addons_public
  add column if not exists transport_schema_version integer not null default 1;

alter table public.account_addons_public
  add column if not exists transport_base_url text;

alter table public.account_addons_public
  add column if not exists transport_secret_ref text;

create index if not exists account_addons_public_transport_secret_idx
  on public.account_addons_public (user_id, transport_secret_ref)
  where transport_secret_ref is not null;
...
set
  ...
  transport_secret_ref = coalesce(transport_secret_ref, secret_ref)
where transport_base_url is null;
...
declare
  ...
  v_transport_secret_payload jsonb := '{}'::jsonb;
begin
  select *
    into v_addon
  ...
  if v_addon.secret_ref is not null and length(trim(v_addon.secret_ref)) > 0 then
    v_secret_payload := public.service_resolve_account_secret(p_user_id, 'addon_credential', v_addon.secret_ref, p_source);
  end if;

  if v_addon.transport_secret_ref is not null and length(trim(v_addon.transport_secret_ref)) > 0 then
    v_transport_secret_payload := public.service_resolve_account_secret(p_user_id, 'addon_credential', v_addon.transport_secret_ref, p_source);
  end if;
  ...
  return jsonb_build_object(
    ...
    'secret_payload', coalesce(v_secret_payload, '{}'::jsonb),
    'transport_schema_version', coalesce(v_addon.transport_schema_version, 1),
    'transport_base_url', coalesce(v_addon.transport_base_url, v_addon.base_url),
    'transport_secret_payload', coalesce(v_transport_secret_payload, '{}'::jsonb)
  );
end;
$$;

revoke all on function public.service_get_account_addon_transport(uuid, uuid, text) from public;
grant execute on function public.service_get_account_addon_transport(uuid, uuid, text) to authenticated;
