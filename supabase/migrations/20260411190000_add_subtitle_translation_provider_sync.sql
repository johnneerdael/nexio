...
-- select public.account_settings_v2_default_payload()#>'{integrations,subtitleTranslation}';
-- select public.sync_pull_account_snapshot(6)->'settings'->'integrations' ? 'subtitleTranslation';
-- select public.sync_pull_account_snapshot(5)->'settings'->'integrations' ? 'subtitleTranslation';
...
begin
  if v_contract_version not in (1, 2, 5, 6) then
...
begin
  ...
  v_settings := case
    when v_contract_version in (2, 5, 6) then public.account_settings_v2_snapshot_payload(v_settings)
  ...
  if v_contract_version in (5, 6) then
...
begin
  if coalesce(p_contract_version, 1) not in (1, 2, 5, 6) then
