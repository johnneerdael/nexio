-- Add wyzie_api_key to the account_secrets secret type constraint and the
-- sync RPC function whitelists. Adds the wyzie integration ({enabled:true})
-- to the v2 account settings default payload and canonical extraction so
-- the public toggle round-trips while the API key itself lives in the
-- secrets table (matching the pattern used by translation_api_key,
-- mdblist_api_key, omdb_api_key, etc.).
...
-- Deploy the default payload with wyzie integration field.
...
-- Deploy the canonical extraction with wyzie integration field.
  ...
  if public.account_settings_is_v2_payload(v_payload) then
    return jsonb_build_object(
      ...
      'integrations', jsonb_build_object(
        ...
        'wyzie',
          coalesce(v_defaults#>'{integrations,wyzie}', '{}'::jsonb)
          || coalesce(v_payload#>'{integrations,wyzie}', '{}'::jsonb),
...
-- select public.account_settings_v2_default_payload()#>'{integrations,wyzie}';
-- select public.account_settings_extract_canonical_v2(
--   '{"schemaVersion":2,"integrations":{"wyzie":{"enabled":false}}}'::jsonb
-- )#>'{integrations,wyzie}';
-- select public.sync_set_account_secret(
--   'wyzie_api_key', 'integration:wyzie',
--   '{"value":"wyzie-test"}'::jsonb, 'wyzie-…test', 'configured', 'app'
-- );
